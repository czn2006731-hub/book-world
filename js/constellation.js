...
let ctaClickHandler = null;
let panelCloseClickHandler = null;
let canvas;
...
function buildOrbs(nebulaType) {
  ...
  books.forEach(function(book,i) {
    ...
    shellParticleSystems.push(shellParticles);
...
function animate() {
  ...
  // 大气层/辉光跟随 + 明暗
  glowSprites.forEach(function(child) {
  ...
  // ── 球形保护壳跟随 + 明暗 + 时间更新 ──
  shellMeshes.forEach(function(child) {
    var p = child.userData.parentOrb;
    child.position.copy(p.position);
    child.visible = p.visible;
    // 跟随行星明暗
  ...
  // ── 壳体附着粒子动画 ──
  shellParticleSystems.forEach(function(child) {
    var p = child.userData.parentOrb;
    child.position.copy(p.position);
    child.visible = p.visible;
...
function onPointerUp(e) {
    ...
    var dx = clientX - pointerDownPos.x;
    var dy = clientY - pointerDownPos.y;
    pointerDownPos = null;
    if (Math.sqrt(dx * dx + dy * dy) > 8) return;
    var h = getHits({ clientX: clientX, clientY: clientY });
...
window.initConstellation = function(nebulaType) {
  ...
  if(panelClose) {
    if(panelCloseClickHandler) panelClose.removeEventListener('click', panelCloseClickHandler);
    panelCloseClickHandler = function() { rightPanel.classList.add('hidden'); clearOrbitRing(); };
    panelClose.addEventListener('click', panelCloseClickHandler);
  }

  if(ctaBtn) {
    if(ctaClickHandler) ctaBtn.removeEventListener('click', ctaClickHandler);
    ...
    ctaBtn.addEventListener('click', ctaClickHandler);
  }
...
window.destroyConstellation = function() {
  ...
  if(ctaBtn && ctaClickHandler) { ctaBtn.removeEventListener('click', ctaClickHandler); ctaClickHandler = null; }
  var panelClose = document.getElementById('panel-close');
  if(panelClose && panelCloseClickHandler) { panelClose.removeEventListener('click', panelCloseClickHandler); panelCloseClickHandler = null; }
  ...
  shellMeshes=[]; shellParticleSystems=[]; glowSprites=[];
