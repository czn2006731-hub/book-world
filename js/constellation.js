...
const NEBULA_NAMES = { scifi:'科幻宇宙', xianxia:'修仙宇宙', romance:'文学宇宙' };
const NEBULA_TITLE_COLORS = { scifi:'#38bdf8', xianxia:'#f59e0b', romance:'#f472b6' };

const COVER_IMAGES = {
  'x1': 'assets/covers/douPoCangQiong.jpg',
  'x2': 'assets/covers/fanRenXiuXianZhuan.jpg',
  'x3': 'assets/covers/zheTian.jpg',
  'x4': 'assets/covers/wanMeiShiJie.jpg',
  'x5': 'assets/covers/xianNi.jpg',
  'x6': 'assets/covers/jianLai.jpg'
};
...
let rightPanel, panelTitle, panelAuthor, panelCover, ctaBtn, paperNote;
...
// ─── 专属纹理：仙逆 → 阴阳双极 ───
function generateYinYangTexture(hexColor, seed) {
  const c = document.createElement('canvas');
  c.width = 1024; c.height = 512;
  const ctx = c.getContext('2d');
  const [baseR,baseG,baseB] = hexToRGB(hexColor);
  ...
  function noise(x, y, s) {
    const nx = (x/1024*s)%N, ny = (y/512*s)%N;
    const ix = nx|0, iy = ny|0, fx = nx-ix, fy = ny-iy;
    return grid[iy*N+ix]*(1-fx)*(1-fy) + grid[iy*N+((ix+1)%N)]*fx*(1-fy)
         + grid[((iy+1)%N)*N+ix]*(1-fx)*fy + grid[((iy+1)%N)*N+((ix+1)%N)]*fx*fy;
  }
  function fbm(x, y, oct, s) {
    let v = 0, a = 0.5, f = 1;
  ...
  for (let py=0; py<512; py++) {
    for (let px=0; px<1024; px++) {
      const i = (py*1024+px)*4;
      const dx = px - cx, dy = (py - cy) * 2;
      const angle = Math.atan2(dy, dx);
      const dist = Math.sqrt(dx*dx + dy*dy);
      const swirl = angle + dist * 0.008 + fbm(px, py, 3, 2) * 2.5;
      const yin = Math.sin(swirl) * 0.5 + 0.5;
      const warp = fbm(px + Math.cos(swirl)*100, py + Math.sin(swirl)*80, 4, 2.5);
      const detail = fbm(px + warp*120, py + warp*100, 3, 4) * 0.3;
      const bright = Math.max(0, Math.min(1, yin + detail));
      const dark = Math.max(0, Math.min(1, 1 - yin + detail));
      const edge = Math.max(0, 1 - dist / 200);
      d[i]   = Math.min(255, (baseR * dark * 0.3 + baseR * bright * 0.7) * (0.6 + edge*0.4));
      d[i+1] = Math.min(255, (baseG * dark * 0.3 + baseG * bright * 0.7) * (0.6 + edge*0.4));
      d[i+2] = Math.min(255, (baseB * dark * 0.3 + baseB * bright * 0.7) * (0.6 + edge*0.4));
      d[i+3] = 255;
    }
  }
  ctx.putImageData(imgData, 0, 0);
  ...
  glow.addColorStop(0, `rgba(${baseR},${baseG},${baseB},0.4)`);
  glow.addColorStop(0.5, `rgba(${baseR},${baseG},${baseB},0.1)`);
...
// ─── 专属纹理：斗破苍穹 → 火莲焚天 ───
function generateFireLotusTexture(hexColor, seed) {
  const c = document.createElement('canvas');
  c.width = 1024; c.height = 512;
  const ctx = c.getContext('2d');
  const [baseR,baseG,baseB] = hexToRGB(hexColor);
  ...
  for (let py=0; py<512; py++) {
    for (let px=0; px<1024; px++) {
      const i = (py*1024+px)*4;
      const warp = fbm(px, py, 3, 2.5);
      const flame = fbm(px + warp*180, py + warp*150, 5, 3);
      const heat = fbm(px + 400, py + 300, 3, 5);
      const base = 0.3 + flame * 0.7;
      const hot = heat > 0.55 ? (heat - 0.55) * 3 : 0;
      d[i]   = Math.min(255, baseR * (0.5 + base * 0.5) + hot * 80);
      d[i+1] = Math.min(255, baseG * (0.3 + base * 0.3) + hot * 40);
      d[i+2] = Math.min(255, baseB * (0.2 + base * 0.2));
      d[i+3] = 255;
    }
  }
  ctx.putImageData(imgData, 0, 0);
  ...
  for (let p = 0; p < petals; p++) {
    const angle = (p / petals) * Math.PI * 2 + rand() * 0.3;
    const len = 60 + rand() * 80;
    const px2 = cx + Math.cos(angle) * len;
    const py2 = cy + Math.sin(angle) * len * 0.6;
    const petalGrad = ctx.createRadialGradient(px2, py2, 0, px2, py2, 30 + rand()*20);
    petalGrad.addColorStop(0, `rgba(255,${120+rand()*60|0},0,${0.3+rand()*0.2})`);
    petalGrad.addColorStop(0.6, `rgba(200,50,0,${0.15+rand()*0.1})`);
    petalGrad.addColorStop(1, 'rgba(100,0,0,0)');
    ctx.fillStyle = petalGrad;
    ctx.beginPath();
    ctx.ellipse(px2, py2, 25+rand()*15, 15+rand()*10, angle, 0, Math.PI*2);
    ctx.fill();
  }

  // 莲心光点
  const coreGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, 40);
  coreGrad.addColorStop(0, 'rgba(255,220,100,0.5)');
  coreGrad.addColorStop(0.5, 'rgba(255,100,0,0.2)');
  ...
  // 火焰粒子
  for (let i=0; i<20; i++) {
    const fx = cx + (rand()-0.5)*300;
    const fy = cy + (rand()-0.5)*180;
    const fg = ctx.createRadialGradient(fx, fy, 0, fx, fy, 2+rand()*3);
    fg.addColorStop(0, `rgba(255,${150+rand()*100|0},0,${0.4+rand()*0.3})`);
    fg.addColorStop(1, 'rgba(200,0,0,0)');
    ctx.fillStyle = fg;
    ctx.beginPath(); ctx.arc(fx, fy, 2+rand()*3, 0, Math.PI*2); ctx.fill();
  }
...
// ─── 专属纹理：凡人修仙传 → 风雷双翼 ───
function generateWindThunderTexture(hexColor, seed) {
  const c = document.createElement('canvas');
  c.width = 1024; c.height = 512;
  const ctx = c.getContext('2d');
  const [baseR,baseG,baseB] = hexToRGB(hexColor);
  ...
  for (let py=0; py<512; py++) {
    for (let px=0; px<1024; px++) {
      const i = (py*1024+px)*4;
      const warp = fbm(px, py, 3, 2);
      const wind = fbm(px + warp*200, py + warp*100, 5, 3);
      const base = 0.3 + wind * 0.7;
      d[i]   = Math.min(255, baseR * (0.6 + base * 0.4));
      d[i+1] = Math.min(255, baseG * (0.7 + base * 0.3));
      d[i+2] = Math.min(255, baseB * (0.8 + base * 0.2));
      d[i+3] = 255;
    }
  }
  ctx.putImageData(imgData, 0, 0);
  ...
  for (let side = -1; side <= 1; side += 2) {
    ...
    for (let feather = 0; feather < 8; feather++) {
      const angle = side === -1 ? -0.4 + feather * 0.12 : Math.PI + 0.4 - feather * 0.12;
      const len = 80 + feather * 25 + rand() * 15;
      const fx = baseX + Math.cos(angle) * len;
      const fy = cy + Math.sin(angle) * len * 0.5;
      const featherGrad = ctx.createRadialGradient(fx, fy, 0, fx, fy, 12 + rand()*8);
      featherGrad.addColorStop(0, `rgba(${baseR},${baseG},${baseB},${0.2+rand()*0.15})`);
      featherGrad.addColorStop(0.7, `rgba(${baseR},${baseG},${baseB},${0.08})`);
      featherGrad.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = featherGrad;
      ctx.beginPath();
      ctx.ellipse(fx, fy, 15+rand()*10, 6+rand()*4, angle, 0, Math.PI*2);
      ctx.fill();
    }
  }
  ...
  // 雷电纹理
  for (let i=0; i<6; i++) {
    const sx = cx + (rand()-0.5)*400;
    const sy = cy + (rand()-0.5)*250;
    ctx.strokeStyle = `rgba(200,220,255,${0.2+rand()*0.15})`;
  ...
  coreGrad.addColorStop(0, `rgba(${baseR},${baseG},${baseB},0.35)`);
  coreGrad.addColorStop(0.6, `rgba(${baseR},${baseG},${baseB},0.1)`);
  coreGrad.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = coreGrad;
...
const CUSTOM_TEXTURE_MAP = {
  'x1': generateFireLotusTexture,   // 斗破苍穹 → 火莲焚天
  'x2': generateWindThunderTexture, // 凡人修仙传 → 风雷双翼
  'x4': generateLavaTexture,        // 完美世界
  'x5': generateYinYangTexture,     // 仙逆 → 阴阳双极
...
function buildOrbs(nebulaType) {
  ...
  // ── 3D 专属 Sprite 文字标签 —— "探索人生" ──
  var centralLabelCanvas = document.createElement('canvas');
  centralLabelCanvas.width = 512; centralLabelCanvas.height = 128;
  var clCtx = centralLabelCanvas.getContext('2d');
  clCtx.font = 'bold 36px "PingFang SC", "Microsoft YaHei", sans-serif';
  clCtx.textAlign = 'center';
  clCtx.textBaseline = 'middle';
  clCtx.fillStyle = '#ffd700';
  clCtx.shadowColor = 'rgba(255, 215, 0, 0.8)';
  clCtx.shadowBlur = 12;
  clCtx.fillText('探索人生', 256, 64);
  var centralLabelTex = new THREE.CanvasTexture(centralLabelCanvas);
  var centralLabelMat = new THREE.SpriteMaterial({ map: centralLabelTex, transparent: true, depthWrite: false });
  var centralLabel = new THREE.Sprite(centralLabelMat);
  centralLabel.scale.set(7.5, 1.875, 1);
  centralLabel.position.set(0, starRadius * 1.6 + 0.8, 0);
  orbGroup.add(centralLabel);
...
function setGameProgress(bookId, progress) {
  var data = getExplorationData();
  if (!data[bookId]) data[bookId] = { progress: 0, played: false };
  data[bookId].played = true;
  data[bookId].progress = Math.max(data[bookId].progress || 0, Math.min(100, Math.round(progress)));
  ...
  if(progressText) progressText.textContent = data[bookId].progress + '%';
  if(progressFill) progressFill.style.width = data[bookId].progress + '%';
...
function selectBook(book) {
  ...
  // 按钮颜色跟随星云
  var nebColors = {
    scifi: { glow:'rgba(56,189,248,0.25)', border:'rgba(56,189,248,0.4)', hover:'rgba(56,189,248,0.15)' },
    xianxia: { glow:'rgba(245,158,11,0.25)', border:'rgba(245,158,11,0.4)', hover:'rgba(245,158,11,0.15)' },
    romance: { glow:'rgba(244,114,182,0.25)', border:'rgba(244,114,182,0.4)', hover:'rgba(244,114,182,0.15)' }
  };
  var nc = nebColors[currentNebulaType] || nebColors.scifi;
  ctaBtn.style.setProperty('--btn-glow', nc.glow);
  ctaBtn.style.setProperty('--btn-border', nc.border);
  ctaBtn.style.setProperty('--btn-hover', nc.hover);

  paperNote.innerHTML = '<div class="tape"></div><div class="note-icon">📖</div>「'+book.quote+'」<br><span style="font-size:11px;opacity:0.6">—— 《'+book.title+'》</span><button id="tts-replay-btn" class="tts-replay-btn" onclick="playQuoteAudioDirect(\'' + book.quote.replace(/'/g, "\\'") + '\', \'' + book.id + '\')" title="播放语音">🔊</button>';

  // 不再自动播放名言语音，点击按钮才播放
  // if (typeof playQuoteAudio === 'function') {
  //   playQuoteAudio(book.quote, book.id);
  // }
...
function clearOrbitRing() {
  if (orbitRing) { if(scene) scene.remove(orbitRing); orbitRing.geometry.dispose(); orbitRing.material.dispose(); orbitRing=null; }
  if (orbitRingGlow) { if(scene) scene.remove(orbitRingGlow); orbitRingGlow.geometry.dispose(); orbitRingGlow.material.dispose(); orbitRingGlow=null; }
  selectedMesh = null;
  selectedBook = null;
  isFlying = false;
  // 重置所有行星明暗
...
function onPointerUp(e) {
  ...
  // 检测 3D 中央恒星点击 (点击恒星进入探索人生)
  if (centralStar && camera) {
    var mouse = new THREE.Vector2();
    mouse.x = (e.clientX / window.innerWidth) * 2 - 1;
    mouse.y = -(e.clientY / window.innerHeight) * 2 + 1;
    var raycaster = new THREE.Raycaster();
    raycaster.setFromCamera(mouse, camera);
    var starHits = raycaster.intersectObject(centralStar);
    if (starHits.length > 0) {
      switchScreen('explore-life');
      return;
    }
  }
...
window.initConstellation = function(nebulaType) {
  ...
  if(titleEl) {
    titleEl.textContent = NEBULA_NAMES[nebulaType] || '书籍宇宙';
    titleEl.style.color = NEBULA_TITLE_COLORS[nebulaType] || '#ffffff';
  }
  ...
  // 显示恒星点击区域
  var starClickArea = document.getElementById('star-click-area');
  if(starClickArea) {
    starClickArea.style.display = 'block';
  }

  var panelClose = document.getElementById('panel-close');
  if(panelClose) { panelClose.onclick = function() { rightPanel.classList.add('hidden'); clearOrbitRing(); }; }
...
window.destroyConstellation = function() {
  ...
  // 隐藏恒星点击区域
  var starClickArea = document.getElementById('star-click-area');
  if(starClickArea) starClickArea.style.display = 'none';
};

// ══════════════════════════════════════════════
//  CTA 按钮全局 handler（inline onclick 调用）
// ══════════════════════════════════════════════

window.handleEnterBook = function() {
  if(!selectedBook) return;
  ctaBtn.innerHTML = '<span class="cta-icon">&#x2728;</span> 进入中...';
  markBookPlayed(selectedBook.id);
  ...
  var bookId = currentNebulaType + '.' + (map[selectedBook.title] || selectedBook.title);
  
  // 直接打开存档选择弹窗
  openSaveSlotsModal(bookId, selectedBook);
  
  setTimeout(function() {
    if(ctaBtn) ctaBtn.innerHTML = '<span class="cta-icon">&#x1F4D6;</span> 选择存档';
  }, 2500);
};
