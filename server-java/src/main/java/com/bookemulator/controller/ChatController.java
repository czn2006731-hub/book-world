...
public class ChatController {
    ...
    /**
     * 动态生成选择项（4个：1个原著 + 3个AI推演）
     */
    @PostMapping("/generate-choices")
    public Map<String, Object> generateChoices(@RequestBody ChatRequest request) {
        return aiService.generateChoices(request.getContext(), request.getOriginalChoice(), request.getHistory());
    }
     ...
     */
    @PostMapping("/narrate")
    public String narrate(@RequestBody ChatRequest request) {
        return aiService.narrateResult(request.getChoiceLabel(), request.getContext());
    }
