...
router.post('/generate-choices', async (req, res) => {
  try {
    const { context, originalChoice, history } = req.body;

    const prompt = `你是一个《凡人修仙传》原著小说级的大模型主策划。请基于《凡人修仙传》独有的硬核现实主义修仙世界观、当下上下文和韩立稳健性格脉络，生成 4 个富有凡人修仙特色的玩家行动选项。\n` +
      `【故事背景/上下文】：${context || ''}\n` +
      `【原著推荐走向】：${originalChoice ? (typeof originalChoice === 'object' ? originalChoice.choice : originalChoice) : '顺应原著走向'}\n` +
      `【历史玩家行动】：${history || '无'}\n\n` +
      `【凡人修仙传核心规则】：\n` +
      ...
      `4. 绝不要带有任何 Markdown 标记（如反引号包裹的代码块）。`;
    ...
    res.status(500).json({ error: '无法生成动态选项' });
  }
});
...
router.post('/narrate', async (req, res) => {
  try {
    const { choiceLabel, context } = req.body;

    const prompt = `玩家在《凡人修仙传》游戏中做出了选择：「${choiceLabel || '特殊决定'}」。\n` +
      `故事上下文：${context || ''}\n\n` +
    ...
    console.error('叙事描述结果错误:', error);
    res.status(500).send('【叙事发生变故】变幻莫测的风云席卷了天地……');
  }
});
